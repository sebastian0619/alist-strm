<template>
  <div class="strm-health-panel">
    <a-card class="health-card" :bordered="false">
      ...
      <!-- 批量替换STRM文件内容 -->
      <a-card title="批量替换STRM文件内容" style="margin-top: 20px;">
        <a-form layout="vertical">
          ...
          <a-form-item>
            <a-button 
              ...
              :disabled="!batchReplace.searchText || isScanning"
            >
...
const previewBatchReplace = async () => {
  ...
  try {
    ...
    const targetPaths = batchReplace.value.onlyInvalid ? 
      filteredProblems.value.filter(p => p.type === 'invalid_strm').map(p => p.path) : 
      null
